...
def explicit_cython(np.ndarray[np.float_t,ndim=1] u, float kappa, float dt, float dz, np.ndarray[np.float_t,ndim=1] term_const, 
    ...
    #u_out = []
    #u_out.append(u_view.copy())
    
    for i in range(len_plot):
        for k in range(plot_view[i], plot_view[i+1]):
            un_view = u_view
            #for j in range(1, nz-1):
            for j in prange(1, nz-1, nogil=True, schedule="guided"):
